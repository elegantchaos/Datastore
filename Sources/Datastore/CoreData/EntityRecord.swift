...
public class EntityRecord: NSManagedObject {
    ...
    func add(_ value: String, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: StringProperty = add(key: key, type: type ?? .string) {
            property.value = value
        }
    }
    
    func add(_ value: Int64, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: IntegerProperty = add(key: key, type: type ?? .integer) {
            property.value = value
        }
    }
    
    func add(_ value: Bool, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: BooleanProperty = add(key: key, type: type ?? .boolean) {
            property.value = value
        }
    }
    
    func add(_ value: Double, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: DoubleProperty = add(key: key, type: type ?? .double) {
            property.value = value
        }
    }
    
    func add(_ value: Date, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: DateProperty = add(key: key, type: type ?? .date) {
            property.value = value
        }
    }
    
    func add(_ value: Data, key: Key, type: DatastoreType?, store: Datastore) {
        if let property: DataProperty = add(key: key, type: type ?? .data) {
            property.value = value
        }
    }
    
    func add(_ value: EntityRecord, key: Key, type: DatastoreType?, store: Datastore) {
    ...
    func add<R>(key: Key, type: DatastoreType) -> R? where R: NamedProperty {
